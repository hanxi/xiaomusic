...
5. **歌曲续播功能**: 播放到歌单末尾时自动搜索并播放相同歌手的歌曲
6. **OpenAPI接口系统**: 支持通过外部API进行音乐搜索和播放
7. **智能语音指令分析**（可选）: 支持对用户语音指令的智能分析，自动提取歌名、歌手名（需用户配置API密钥，默认不启用）
...
- 通过匹配度算法优化搜索结果排序（多平台聚合搜索场景）
- 使用智能语音指令分析功能（可选），自动识别歌曲名和歌手名信息
...
### 智能语音指令分析（可选功能）
- 使用AI技术分析用户语音指令，自动提取歌曲名和歌手名信息
- 默认不启用，需用户主动配置API密钥后方可使用

### 语音指令扩展
- 新增【在线播放】语音控制指令
- 新增【播放歌手】语音控制指令，可播放指定歌手的歌曲列表
...
- 【在线播放+关键词(歌手、歌曲名)】，会直接调用接口或插件，搜索关键词,返回匹配后的第一个资源进行播放。比如说：【在线播放】林俊杰||江南||林俊杰+江南。
